...
describe CourseOldAccreditedBodyChangeEmailMailer, type: :mailer do
  let(:course) { create(:course, :with_accrediting_provider) }
  let(:user) { create(:user) }
  let(:mail) { described_class.course_old_accredited_body_change_email(course, user, DateTime.new(2001, 2, 3, 4, 5, 6)) }
  ...
  context "sending an email to a user" do
    it "sends an email with the correct template" do
      expect(mail.govuk_notify_template).to eq(Settings.govuk_notify.course_old_accredited_body_change_email_template_id)
    end
    ...
    it "includes the new accrediting provider name in the personalisation" do
      expect(mail.govuk_notify_personalisation[:new_accrediting_body]).to eq(course.accrediting_provider.provider_name)
    end

    it "includes the datetime for the withdrawl in the personalisation" do
      expect(mail.govuk_notify_personalisation[:course_change_datetime]).to eq("4:05am on 3 February 2001")
    end
  end
end
